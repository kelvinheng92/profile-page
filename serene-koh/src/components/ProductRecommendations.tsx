...
const categoryColors: Record<string, string> = {
  ...
  "Property & Mortgage": "bg-blue-700",
  "Insurance & Protection": "bg-amber-600",
  "CPF & Tax Planning": "bg-emerald-700",
  "Banking & Lifestyle": "bg-[#1a1a2e]",
};
        ...
        {products.map((cat) => (
          <div key={cat.category} className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
            <div className={`${categoryColors[cat.category] ?? "bg-gray-700"} px-5 py-3 flex items-center justify-between`}>
              ...
              <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${cat.priority === "PRIMARY" ? "bg-white/30 text-white" : "bg-white/15 text-white/75"}`}>
                {cat.priority}
              </span>
            </div>
